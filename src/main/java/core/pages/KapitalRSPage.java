...
public class KapitalRSPage extends BasePage {
    ...
    @FindBy(xpath = "//span[@class='errorMessage' and text()='Email ili broj telefona već postoji. Molimo Vas koristite drugi email ili broj telefona.']")
    ...
    @FindBy(xpath = "//div[@class='logo-img-kapitalrs']")
    ...
    @FindBy(xpath = "//p/a[text()='Politiku privatnosti']")
    public WebElement headerPolitikaPrivatnostiLink;

    @FindBy(xpath = "//p/a[text()='Uslove i odredbe']")
    public WebElement headerUsloviIOdredbeLink;

    @FindBy(xpath="//a[text()='kliknite ovde']")
    public WebElement klikniteOvdeLink;

    @FindBy(xpath = "//a[contains(text(), 'Već imate nalog?')]")
    public WebElement vecImateNalogLink;

    @FindBy(xpath = "//div/a[text()='Kontakt']")
    public WebElement kontaktirajteNasLink;

    @FindBy(xpath = "//p/a[text()='Politika o zaštiti privatnosti']")
    public WebElement footerPolitikaPrivatnostiLink;

    @FindBy(xpath = "//p/a[text()='Upozorenje o riziku']")
    public WebElement footerUpozorenjeORizikuLink;

    @FindBy(xpath = "//p/a[text()='609970']")
    public WebElement footerFCALink;

    @FindBy(xpath = "//p/a[text()='ABN: 33 614 683 831 | AFSL: 493520.']")
    public WebElement footerASICLink;

    @FindBy(xpath = "//p/a[text()='GB21026472']")
    ...
    String[] sameNamesErrorMessages = {"Vaše ime mora da se razlikuje od vašeg prezimena.",
            "Vaše ime mora da se razlikuje od vašeg prezimena."};

    public String msgAlrRegEmailAdd = "Email ili broj telefona već postoji. Molimo Vas koristite drugi email ili broj telefona.";

    public String msgAlrRegPhone = "Email ili broj telefona već postoji. Molimo Vas koristite drugi email ili broj telefona.";
    ...
    public void clickHereLink(){
        ElementActions.click(klikniteOvdeLink,"click here link");
    }
    ...
    public void assertSecondStepErrorMessage(String parameter){
        Assert.assertEquals(ElementActions.getText(By.xpath("(//span[@class='selectErrorMessage'])[position()=1]"), "error message"), TestData.secondStepErrorMessageKapitalRS);
        switch (parameter){
            case "age":
    ...
    public void assertSecondStepErrorMessageAllParameters(){
        for (int i = 1; i < 6; i++){
            Assert.assertEquals(ElementActions.getText(By.xpath("(//span[@class='selectErrorMessage'])[position()={index}]".replace("{index}", String.valueOf(i))), "second step error message"), TestData.secondStepErrorMessageKapitalRS);
        }
        assertBorderColor(age, "border-color", TestData.redBorderColor);
        assertBorderColor(annual, "border-color", TestData.redBorderColor);
        assertBorderColor(saving, "border-color", TestData.redBorderColor);
    ...
    public void fillTheFormOnTheSecondStepWithWrongData(String firstNameData, String lastNameData, String emailAddress,
        ...
        selectAge("-- Izaberite --");
        selectAnnualIncome(annualData);
        selectAnnualIncome("-- Izaberite --");
        selectSaving(savingData);
        selectSaving("-- Izaberite --");
        selectKnowledge(knowledgeData);
        selectKnowledge("-- Izaberite --");
        selectLanguage(languageData);
        selectLanguage("-- Izaberite --");
        clickSubmitBtn2nd();
    }
    ...
    public void checkIDidntReceiveTheCodeLinkKapitalRS(){
        ElementActions.click(didNotGetToken, "I didn't receive the code");
        Assert.assertEquals(ElementActions.getText(codeIsSent, "weSentYouTheCodeAgainMessage"), "Ponovo smo vam poslali kod");
    }

    public void assertErrorMessageForWrongSmsCodeKapitalRS(){
        Assert.assertEquals(ElementActions.getText(incorrectTokenMsg, "smsFieldErrorMessage"), "ups... taj kod je pogresan . Molimo vas pokusajte ponovo");
